  ...
  public formatForJenkins(
    ...
    if (result.codeContext.fileName) {
      report.codeContext = {
        ...
        ...(result.codeContext.lineNumber !== undefined && {
          line: result.codeContext.lineNumber,
        }),
        ...(result.codeContext.functionName && {
          function: result.codeContext.functionName,
        }),
        ...(options?.includeCodeSnippet &&
          result.codeContext.codeSnippet && {
...
pipeline {
   ...
   */
  public formatForJenkinsConsole(
    result: AdvancedAnalysisResult,
    errorMessage: string
  ): string {
    const lines: string[] = [];
    ...
    if (result.codeContext.fileName) {
      ...
      lines.push(
        `  File: ${result.codeContext.fileName}${result.codeContext.lineNumber ? `:${result.codeContext.lineNumber}` : ''}`
      );
    ...
    lines.push(
      `  Success Probability: ${Math.round(result.confidence * 100)}%`
    );
  ...
  private determineJenkinsStatus(
    ...
    if (severity === 'medium' && confidence >= 0.8) {
      return 'warning';
    }
    if (severity === 'low' && confidence >= 0.7) {
      return 'success';
    }
    return 'warning';
  }
  ...
  private generateBuildRecommendation(
    result: AdvancedAnalysisResult
  ): 'retry' | 'investigate' | 'fix_required' | 'critical_fix' {
    const confidence = result.confidence;
    const severity = result.severity;
    const category = result.category.toLowerCase();
    ...
    if (
      confidence >= 0.8 &&
      (category.includes('timeout') || category.includes('network'))
    ) {
      return 'retry';
    }
  ...
  private calculateFixTime(result: AdvancedAnalysisResult): string {
    ...
    if (category.includes('timeout') && confidence >= 0.8) {
      return '2-5 minutes';
    }
    if (category.includes('network') && confidence >= 0.7) {
      return '5-10 minutes';
    }
    if (category.includes('undefined') || category.includes('reference')) {
      return '10-30 minutes';
    }
    if (category.includes('syntax')) {
