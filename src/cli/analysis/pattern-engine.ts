...
export class AdvancedPatternEngine {
  ...
  public analyzeError(
    ...
    const codeContext = this.analyzeCodeContext(
      errorMessage,
      filePath,
      stackTrace
    );
    ...
    const result: AdvancedAnalysisResult = {
      ...
      quickSolution:
        patterns.solutions[0] || 'Review the error details and check your code',
  ...
  private parseStackTrace(
    errorMessage: string
  ): { mainError: string; location: string; callStack: string[] } | undefined {
    const lines = errorMessage.split('\n');

    // Look for stack trace patterns
    const stackTraceStart = lines.findIndex(
      (line) =>
        line.includes('at ') ||
        line.includes('Error:') ||
  ...
  private identifyErrorType(errorMessage: string): string {
    const patterns = [
      ...
      {
        regex:
          /locator.*timeout|waiting.*timeout.*locator|Timeout.*waiting for locator/i,
      ...
      {
        regex:
          /element.*not.*attached|locator.*not.*attached|element.*detached/i,
      ...
      {
        regex:
          /NoSuchElementException|element.*not.*found|Unable to locate element/i,
      ...
      {
        regex:
          /TypeError.*undefined|Cannot read.*undefined|undefined.*is not an object/i,
      ...
      {
        regex:
          /import.*error|export.*error|Invalid or unexpected token.*import/i,
    ...
    for (const pattern of patterns) {
      if (
        pattern.regex.test(errorMessage) &&
        pattern.weight > bestMatch.weight
      ) {
  ...
  private extractElementDetails(errorMessage: string): {
    ...
    if (errorMessage.includes('clickable')) {
      details.expectedState = 'clickable';
    }
    if (errorMessage.includes('visible')) {
      details.expectedState = 'visible';
    }
    if (errorMessage.includes('hidden')) {
      details.expectedState = 'hidden';
    }
    ...
    for (let i = stackTraceStart; i < lines.length; i++) {
      ...
      if (line.startsWith('at ')) {
        ...
        const testFilePattern =
          /at\s+(?:.*?\s+)?\(?([^:]+\.(spec|test)\.(ts|js|tsx|jsx)):(\d+):(\d+)\)?/;
   ...
   */
  private analyzeCodeContextEnhanced(
    errorMessage: string,
    filePath?: string,
    stackTrace?: any
  ): any {
    ...
    if (enhancedTrace) {
      ...
      if (enhancedTrace.filePath && existsSync(enhancedTrace.filePath)) {
        try {
          ...
          if (
            enhancedTrace.lineNumber > 0 &&
            enhancedTrace.lineNumber <= lines.length
          ) {
            ...
            context.functionName = this.findContainingFunction(
              lines,
              enhancedTrace.lineNumber
            );
          }
        } catch (error) {
      ...
      const fileMatches = errorMessage.match(
        /([^\/\s]+\.(js|ts|jsx|tsx|py|java|cpp)):(\d+)/g
      );
  ...
  private findRelatedFiles(errorMessage: string, fileName?: string): string[] {
    ...
    const filePattern =
      /([a-zA-Z0-9_\-\.\/]+\.(js|ts|jsx|tsx|json|yaml|yml|config))/g;
  ...
  private generatePatterns(
    ...
    const pattern = patterns[errorType] || {
      ...
      solutions: [
        ...
        'Check documentation for related functionality',
      ],
    };

    // Enhance solutions based on code context
    if (codeContext?.fileName && codeContext?.lineNumber) {
      pattern.solutions.unshift(
        `Check line ${codeContext.lineNumber} in ${codeContext.fileName}`
      );
    }
  ...
  private calculateConfidence(
    ...
    if (
      errorMessage.includes('webServer') &&
    ...
    if (errorMessage.includes('TypeError') && errorMessage.includes('null')) {
      confidence += 0.3;
    }

    // Boost confidence for stack trace availability
    if (stackTrace?.callStack.length > 0) {
      confidence += 0.1;
    }
  ...
  private determineSeverity(
    ...
    if (errorMessage.includes('syntax') || errorMessage.includes('typo')) {
      return 'low';
    }
