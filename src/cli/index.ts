...
program
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (options.quiet && options.verbose) {
      console.error(
        chalk.red('Error: Cannot use both --quiet and --verbose flags')
      );
      process.exit(1);
    }

    // Set global log level based on flags
    process.env.LUMOS_LOG_LEVEL = options.quiet
      ? 'error'
      : options.verbose
        ? 'debug'
        : 'info';
...
program
  ...
  .option(
    '--suite <type>',
    'Test suite type (jest, vitest, playwright)',
    'auto'
  )
...
program
  ...
  .option(
    '--preset <type>',
    'Configuration preset (basic, enterprise, custom)',
    'basic'
  )
...
program
  ...
  .option(
    '--operations <types>',
    'Operations to benchmark (all, analyze, validate)',
    'all'
  )
